...
- slowapi 기본 저장소는 **인메모리** → gunicorn `-w 2` 멀티워커에서 카운터가 워커별로 분리되어 실효 한도가 워커 수만큼 늘어남. 다중 인스턴스로 확장 시에도 동일.
...
- 정밀 제한이 필요하면 Redis 백엔드(`limits` 스토리지)로 교체. 단 본 프로젝트(P0v3 단일 인스턴스, 비영리)에선 **근사적 보호로 충분** → 인메모리 + XFF 파싱 권장, Redis는 비범위.
