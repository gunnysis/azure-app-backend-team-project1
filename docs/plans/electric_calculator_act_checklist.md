...
- [x] **방식 확정**: 에어컨 추정치를 `prev_year_usage` 피처로 라우팅(Approach A). *초기 안의 `136(KWH) + (h×0.65×30×0.60)` 직산식 → 기존 타입인지 `estimate_usage` 재사용 + 듀티 0.60·clamp 400으로 정제(상수 3중분기 방지).*
- [x] **구현**: 백엔드 `feature_builder`(`15d1e84`) + 프론트 미러(`2b4b441`).
- [x] **검증**: pytest 48 passed · 정합 그리드 220케이스 0 불일치 · 백엔드 prev 곡선 = 라이브 /predict 검증값.
- [x] **배포·검증 완료**: 백엔드 `deploy.sh`(RuntimeSuccessful) + 프론트 SWA(success). 라이브 스윕 predicted 141→363(0→24h 단조)·predicted>baseline 확인 → **발견 C 해소.**
