...
**✅ 발견 C — 라이브 ML이 `current_usage`(에어컨 습관)를 무시 → 해소·배포완료(2026-06-24)** *(근본원인: current_usage는 학습 라벨. 해결: 에어컨 신호를 prev_year_usage로 라우팅, Approach A. 라이브 검증 predicted 141→363 단조·predicted>baseline. 설계·검증: [`plans/electric_calculator_act_plan.md`](plans/electric_calculator_act_plan.md))* — 아래는 진단 기록:
