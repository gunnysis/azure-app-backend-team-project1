...
- **근본 원인**: 결함은 **백엔드/프론트 코드가 아니라 배포된 Azure ML Designer 모델 자체**. 모델이 `current_usage`(및 `prev_year_usage`)에 **사실상 무가중치** — 월별 기상·계절 피처만으로 거의 상수를 출력(6월≈141). 백엔드 피처 합성·페이로드 변환·응답 추출은 모두 정상 동작(스윕에서 `features_used`가 시간에 따라 정상 변동).
...
- **해결 방향(요청→검토→승인→실행 게이트, 과금 가능성 표기)**:
  1. **ML 재학습**(근본·권장 장기) — `current_usage`/`prev_year_usage`에 반응하도록 모델 재학습·재배포. *Azure ML 컴퓨트·재배포 = 과금/승인 필수.*
  2. **어댑터 블렌딩**(단기 완화·무재학습) — 백엔드 `adapter.estimate`에서 `predicted = 모델baseline + (추정current − 추정baseline)`로 보정. 모델의 계절 기준선은 살리고 에어컨 델타를 휴리스틱으로 주입. *코드 변경만, 추가 과금 없음. 단 "ML 예측"의 의미가 절반 휴리스틱이 됨 — 제품 표기 정합 필요.*
  3. **제품 재포지셔닝**(무코드) — "ML 예측"을 "추정"으로 표기 변경, 비교 UI 조정. 결함을 명시적 한계로 수용.
- **권장**: 단기 **②(블렌딩)** + 장기 **①(재학습)**. ②는 별도 설계 문서(`plans/`)로 **설계→문서화→승인→구현** 진행 예정. **현재 결정 대기.**
